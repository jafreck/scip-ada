...
package body Test_ALI_Parser is
   ...
   begin
      ...
      begin
          Assert_Eq_Nat (E.Line, 7, "basic: Say_Hello line");
          Assert (E.Kind = Procedure_Entity,
             "basic: Say_Hello kind");
         Assert_Eq_Nat (E.Column, 14,
               "basic: Say_Hello col");
          Assert (E.Is_Library_Level,
             "basic: Say_Hello library-level");
            Assert (Actual   => To_String (E.Name),
                 Expected => "Say_Hello",
                 Message  => "basic: Say_Hello name");
         Assert_Eq_Nat (Natural (E.References.Length), 2,
               "basic: Say_Hello ref count");
      end;
   ...
   begin
      ...
      begin
         ...
         Assert (S.Entities (2).Kind = Generic_Package_Instantiation,
                 "generics: My_Stack kind=k");
          Assert (Actual   => To_String (S.Entities (2).Name),
             Expected => "My_Stack",
             Message  => "generics: My_Stack name (no brackets)");
      end;
   end Run_Generics;
   ...
   end Run_Tagged_Types;

   ---------------------------------------------------------------------------
   --  Test: overloaded.ali — overloaded subprograms
   ---------------------------------------------------------------------------

    procedure Run_Overloaded
   is
      A : constant ALI_File :=
        SCIP_Ada.ALI.Parser.Parse (Fixtures & "overloaded.ali");
   begin
      Assert_Eq_Nat (Natural (A.Files.Length), 3,
                     "overloaded: D line count");
      Assert_Eq_Nat (Natural (A.Sections.Length), 2,
                     "overloaded: X section count");
      ...
      begin
         Assert_Eq_Nat (Natural (S.Entities.Length), 8,
               "overloaded: X(1) entity count");

         --  Three overloaded Add functions
         Assert (Actual   => To_String (S.Entities (2).Name),
              Expected => "Add",
              Message  => "overloaded: Add(1) name");
         Assert (Actual   => To_String (S.Entities (3).Name),
              Expected => "Add",
              Message  => "overloaded: Add(2) name");
         Assert (Actual   => To_String (S.Entities (4).Name),
              Expected => "Add",
              Message  => "overloaded: Add(3) name");
          Assert (S.Entities (2).Kind = Function_Entity,
             "overloaded: Add(1) kind=V");

         --  Two overloaded Process procedures
         Assert (Actual   => To_String (S.Entities (5).Name),
              Expected => "Process",
              Message  => "overloaded: Process(1) name");
         Assert (Actual   => To_String (S.Entities (6).Name),
              Expected => "Process",
              Message  => "overloaded: Process(2) name");
          Assert (S.Entities (5).Kind = Procedure_Entity,
             "overloaded: Process(1) kind=U");
      end;
   end Run_Overloaded;
   ...
   ---------------------------------------------------------------------------
    ...
    procedure Run_Operators
   is
      A : constant ALI_File :=
        SCIP_Ada.ALI.Parser.Parse (Fixtures & "operators.ali");
   begin
      Assert_Eq_Nat (Natural (A.Files.Length), 3,
                     "operators: D line count");
      Assert_Eq_Nat (Natural (A.Sections.Length), 2,
                     "operators: X section count");
      ...
      begin
         ...
         --  11V14*"*" — operator multiply
          Assert (Actual   => To_String (S.Entities (7).Name),
             Expected => """*""",
             Message  => "operators: ""*"" name");

         --  12V14*"=" — operator equals
          Assert (Actual   => To_String (S.Entities (8).Name),
             Expected => """=""",
             Message  => "operators: ""="" name");
      end;
   ...
   begin
      ...
      Assert_Eq_Nat (Natural (A.Files.Length), 30,
              "gnat_sample: D line count");
         Assert (Actual   => To_String (A.Files (13).Path),
            Expected => "sample.ads",
            Message  => "gnat_sample: D(13) path");
         Assert (Actual   => To_String (A.Files (14).Path),
            Expected => "sample.adb",
            Message  => "gnat_sample: D(14) path");

      --  X sections: 4 (ada.ads, a-textio.ads, sample.ads, sample.adb)
      Assert_Eq_Nat (Natural (A.Sections.Length), 4,
              "gnat_sample: X section count");
      Assert_Eq_Nat (A.Sections (1).File_Index, 1,
              "gnat_sample: X(1) file");
      Assert_Eq_Nat (A.Sections (3).File_Index, 13,
              "gnat_sample: X(3) file=13");
      Assert_Eq_Nat (A.Sections (4).File_Index, 14,
              "gnat_sample: X(4) file=14");
      ...
      --  First entity: 1K9*Sample
            Assert (Actual   => To_String (A.Sections (3).Entities (1).Name),
               Expected => "Sample",
               Message  => "gnat_sample: entity 1 = Sample");
         Assert (A.Sections (3).Entities (1).Kind = Package_Entity,
            "gnat_sample: Sample kind=K");
         Assert (A.Sections (3).Entities (1).Is_Library_Level,
            "gnat_sample: Sample library-level");

      --  Color entity: 3E9*Color
            Assert (Actual   => To_String (A.Sections (3).Entities (2).Name),
               Expected => "Color",
               Message  => "gnat_sample: entity 2 = Color");
         Assert (A.Sections (3).Entities (2).Kind = Enumeration_Type,
            "gnat_sample: Color kind=E");

      --  Red entity: 3n19*Red
            Assert (Actual   => To_String (A.Sections (3).Entities (3).Name),
               Expected => "Red",
               Message  => "gnat_sample: entity 3 = Red");
         Assert (A.Sections (3).Entities (3).Kind = Enumeration_Literal,
            "gnat_sample: Red kind=n");

      --  Point entity: 5R9*Point
      Assert (A.Sections (3).Entities (6).Kind = Tagged_Record_Type,
            "gnat_sample: Point kind=R");
      ...
      Assert (Natural (A.Sections (4).Entities.Length) >= 3,
       ...
       procedure Run_Gnat_Sample_Main
   is
      A : constant ALI_File :=
   ...
   begin
      ...
      Assert (Natural (A.Sections (1).Entities.Length) >= 5,
            ...
            Assert (Actual   => To_String (A.Sections (1).Entities (1).Name),
               Expected => "Sample",
               Message  => "gnat_main: entity 1 = Sample");

      --  Section 2 (sample_main.adb): contains Sample_Main
            Assert (Actual   => To_String (A.Sections (2).Entities (1).Name),
               Expected => "Sample_Main",
               Message  => "gnat_main: entity 1 = Sample_Main");
         Assert (A.Sections (2).Entities (1).Kind = Procedure_Entity,
            "gnat_main: Sample_Main kind=U");
   ...
   begin
      ...
      begin
         ...
         AUnit.Assertions.Assert
           (False,
            "Parse of missing file should raise Ada.Text_IO.Name_Error");
      exception
         ...
         when others =>
            AUnit.Assertions.Assert
              (False,
               "Parse of missing file raised the wrong exception");
      end;
   end Run_Missing_File;

   procedure Test_Basic (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Basic;
   end Test_Basic;

   procedure Test_Generics (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Generics;
   end Test_Generics;

   procedure Test_Tagged_Types (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Tagged_Types;
   end Test_Tagged_Types;

   procedure Test_Overloaded (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Overloaded;
   end Test_Overloaded;

   procedure Test_Operators (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Operators;
   end Test_Operators;

   procedure Test_Renaming (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Renaming;
   end Test_Renaming;

   procedure Test_Continuation (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Continuation;
   end Test_Continuation;

   procedure Test_Gnat_Sample (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Gnat_Sample;
   end Test_Gnat_Sample;

   procedure Test_Gnat_Sample_Main (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Gnat_Sample_Main;
   end Test_Gnat_Sample_Main;

   procedure Test_Missing_File (T : in out SCIP_Ada.Tests.Fixture) is
      pragma Unreferenced (T);
   begin
      Run_Missing_File;
